...
export default function PremiumCard({
    ...
    return (
        <motion.div
            ...
            className={`premium-card glass-reflection p-5 sm:p-8 border-white/5 relative overflow-hidden group ${className}`}
        >
            ...
            {(title || headerAction) && (
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 relative z-10">
                    <div>
                        ...
                        {subtitle && <p className="text-xs text-muted-foreground mt-2 font-black uppercase tracking-[0.2em]">{subtitle}</p>}
                    </div>
                    {headerAction}
                </div>
            )}
