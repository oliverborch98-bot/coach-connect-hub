...
export default function CoachClientDetail() {
  ...
  const renderOverview = () => (
    <div className="space-y-4">
      ...
      {(client.onboarding_completed || onboarding) && (
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {(client.onboarding_completed ? [
              { label: 'Primært mål', value: client.goal },
              { label: 'Erfaringsniveau', value: client.experience_level },
              { label: 'Træning (timer/uge)', value: client.training_days_per_week ? `${client.training_days_per_week} timer` : null },
              { label: 'Kosttype', value: client.diet_type },
              { label: 'Måltider/dag', value: client.meals_per_day },
              { label: 'Allergier', value: client.allergies },
              { label: 'Skader', value: client.injuries },
              { label: 'Medicin', value: client.medications },
              { label: 'Fødselsdato', value: client.birth_date ? new Date(client.birth_date).toLocaleDateString('da-DK') : null },
              { label: 'Køn', value: client.gender },
              { label: 'Højde', value: client.height_cm ? `${client.height_cm} cm` : null },
            ] : [
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        ...
        {checkins.length === 0 ? (
          ...
          <div className="overflow-x-auto -mx-5 px-5 md:mx-0 md:px-0">
            <div className="grid gap-2 min-w-[600px] md:min-w-0">
    ...
    <div className="space-y-6 max-w-5xl mx-auto px-4 md:px-6 pb-24 md:pb-12">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden md:flex gap-2">
          <button onClick={() => setActiveTab('messages')} className="px-4 py-2 rounded-xl bg-primary/20 text-primary text-xs font-black uppercase tracking-widest hover:bg-primary/30 flex items-center gap-2 border border-primary/20 transition-all">
            <MessageSquare className="h-4 w-4" /> Besked
          </button>
          <button onClick={() => navigate('/coach/calls')} className="px-4 py-2 rounded-xl bg-white/5 border border-white/5 text-xs font-black uppercase tracking-widest text-muted-foreground hover:text-foreground hover:bg-white/10 flex items-center gap-2 transition-all">
