...
export default function ProgramBuilder() {
  ...
  const assignGroupMutation = useMutation({
    mutationFn: async () => {
      if (selectedClients.length === 0) throw new Error('Vælg mindst én klient');
      if (!programName.trim()) throw new Error('Angiv programnavn');
      if (!startDate) throw new Error('Vælg startdato');

      for (const cId of selectedClients) {
        const { data: program, error: pErr } = await supabase.from('training_programs').insert({
          client_id: cId, name: programName, phase, status: 'active', is_template: false, start_date: startDate
        }).select('id').single();
        if (pErr) throw pErr;
    ...
    onSuccess: () => {
      toast.success(`Program tildelt til ${selectedClients.length} klienter`);
      setIsAssignModalOpen(false);
      qc.invalidateQueries({ queryKey: ['coach-clients-list'] });
    },
    onError: (err: any) => toast.error(err.message ?? 'Fejl ved tildeling af gruppe'),
  });
    ...
    <div className="space-y-6 max-w-4xl mx-auto px-4 md:px-6 pb-24 md:pb-12">
                      ...
                      {ex.exercise_id ? (
                        ...
                        <div className="relative">
                          ...
                          {pickingForDay === `${day.id}-${ex.id}` && (
                            <div className="absolute z-10 top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-lg max-h-64 overflow-hidden flex flex-col min-w-[280px] sm:min-w-[400px]">
      ...
      <div className="flex justify-end gap-3 pb-8">
        ...
        <button onClick={() => setIsAssignModalOpen(true)}
          className="px-5 py-2 rounded-lg bg-blue-600/20 text-blue-500 border border-blue-600/50 text-sm font-medium hover:bg-blue-600/30 transition-colors flex items-center gap-2">
          Tildel til gruppe
        </button>
      ...
      <Dialog open={isAssignModalOpen} onOpenChange={setIsAssignModalOpen}>
        <DialogContent className="max-w-md bg-zinc-950 border border-zinc-800">
          <DialogHeader>
            <DialogTitle className="text-xl text-amber-500">Tildel til gruppe</DialogTitle>
            <DialogDescription className="text-zinc-400">
              Vælg klienter og startdato for at tildele "{programName || 'Program'}" til flere på én gang.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-6 pt-4">
            <div className="space-y-3">
              <label className="text-sm font-medium text-zinc-300">Vælg Klienter</label>
              <div className="max-h-48 overflow-y-auto space-y-2 border border-zinc-800 rounded-lg p-3 bg-black custom-scrollbar">
                {clients.map((c: any) => (
                  <label key={c.id} className="flex items-center gap-3 cursor-pointer p-2 hover:bg-zinc-900 rounded-md transition-colors">
                    <input 
                      type="checkbox" 
                      className="w-4 h-4 rounded border-zinc-700 bg-zinc-900 text-blue-600 focus:ring-blue-600 focus:ring-offset-black"
                    ...
                    />
                    <span className="text-sm text-zinc-200">{c.profiles?.full_name ?? 'Ukendt'}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-zinc-300">Startdato for program</label>
              <input 
                type="date" 
                value={startDate}
                onChange={(e) => setStartDate(e.target.value)}
                className="w-full rounded-lg border border-zinc-800 bg-black px-3 py-2 text-sm text-zinc-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none"
              />
            </div>
          </div>
          <div className="flex justify-end gap-3 mt-6">
            <button onClick={() => setIsAssignModalOpen(false)} className="px-4 py-2 rounded-lg border border-zinc-800 text-sm text-zinc-400 hover:bg-zinc-900">
              Annuller
            </button>
            <button onClick={() => assignGroupMutation.mutate()} disabled={assignGroupMutation.isPending || selectedClients.length === 0}
              className="px-5 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition-colors flex items-center gap-2 disabled:opacity-50">
              {assignGroupMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
              Tildel til valgte klienter
            </button>
          </div>
        </DialogContent>
      </Dialog>
