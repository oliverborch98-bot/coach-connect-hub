  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto px-4 md:px-6 pb-24 md:pb-12">
      ...
      <div className="flex flex-wrap gap-2">
        {sections.map(s => (
          <button key={s.id} onClick={() => setActiveSection(s.id)} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium min-h-[44px] ${activeSection === s.id ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-secondary'}`}>
      ...
      {activeSection === 'profile' && (
        <div className="rounded-xl border border-border bg-card p-5 space-y-4">
          ...
          <button onClick={() => updateProfile.mutate()} className="flex items-center gap-1 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium min-h-[44px]">
      ...
      {activeSection === 'habits' && (
        <div className="rounded-xl border border-border bg-card p-5 space-y-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground/70">Disse habits tildeles automatisk når du opretter en ny klient.</p>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium disabled:opacity-50 min-h-[44px]"
            >
