  ...
  const { data: clientProfile } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, current_week, current_phase')
        .eq('user_id', user!.id)
        .single();
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  <div className={`h-8 w-8 rounded-full flex items-center justify-center ${
                    isActive ? 'gold-gradient' : isCompleted ? 'bg-success/20' : 'bg-secondary'
                  }`}>
                    {isCompleted ? (
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      Uge {phase.start_week}–{phase.end_week}
                    </p>
                  </div>
                  {isActive && (
                    <span className="text-[10px] font-medium text-primary px-2 py-0.5 rounded bg-primary/10">AKTIV</span>
                  )}
                </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {phase.focus_items.map((item: string, fi: number) => (
                            <span key={fi} className="text-[10px] px-2 py-0.5 rounded bg-secondary text-secondary-foreground">
                              {item}
                            </span>
                          ))}
                        </div>
