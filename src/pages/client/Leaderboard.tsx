...
export default function Leaderboard() {
  ...
  const { data: clientProfile } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('client_profiles').select('id').eq('user_id', user!.id).single();
    ...
    <div className="space-y-5 max-w-lg mx-auto">
      ...
      {myRank > 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          className="rounded-xl border border-primary/30 bg-primary/5 p-4 flex items-center gap-4 relative overflow-hidden"
        >
          <div className="absolute top-0 right-0 p-2 opacity-10">
            <Trophy className="h-12 w-12 text-primary rotate-12" />
          </div>
          <div className="h-12 w-12 rounded-full royal-blue-gradient flex items-center justify-center shadow-lg shadow-primary/20">
            <span className="text-xl font-bold text-white">#{myRank}</span>
          </div>
          <div className="z-10">
            <p className="text-sm font-bold text-foreground">Din placering</p>
            <p className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5">
              <span className="font-semibold text-primary">{(scores.find(s => s.client_id === clientProfile?.id)?.total_points ?? 0).toLocaleString()} pts</span>
              <span>·</span>
              <span className="flex items-center gap-1">
                <Flame className="h-3 w-3 text-orange-400" />
                {scores.find(s => s.client_id === clientProfile?.id)?.current_streak ?? 0} dages streak
              </span>
            </p>
          </div>
        </motion.div>
