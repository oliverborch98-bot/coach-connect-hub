...
export default function ClientTransformation() {
  ...
  const { data: score } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('accountability_scores').select('*').eq('client_id', clientProfile!.id).single();
      return data;
    },
    enabled: !!clientProfile,
  });
    ...
    <div className="space-y-5 max-w-lg mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-border bg-card p-5 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div><p className="text-2xl font-extrabold">{startWeight || '–'}</p><p className="text-[10px] text-muted-foreground">Startvægt</p></div>
          <div><p className="text-2xl font-extrabold text-primary">{currentWeight || '–'}</p><p className="text-[10px] text-muted-foreground">Nuværende</p></div>
          <div><p className="text-2xl font-extrabold text-success">{lost > 0 ? `-${lost.toFixed(1)}` : lost < 0 ? `+${Math.abs(lost).toFixed(1)}` : '0'}</p><p className="text-[10px] text-muted-foreground">Ændring (kg)</p></div>
        </div>
        {goalWeight > 0 && (
          <div>
      ...
      {(firstPhotos.length > 0 || latestPhotos.length > 0) && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-xl border border-border bg-card p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-xs text-muted-foreground mb-2 text-center">Start</p>
            ...
            <div>
              <p className="text-xs text-muted-foreground mb-2 text-center">Måned {latestMonth}</p>
              <div className="grid gap-2">
