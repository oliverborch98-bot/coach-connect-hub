...
export default function ProgressPhotos() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>, photoType: string) => {
    ...
    if (file.size > MAX_SIZE) {
      toast.error('Filen er for stor. Max 5MB.');
      return;
    }
    if (!ALLOWED_TYPES.includes(file.type)) {
      toast.error('Kun JPG, PNG og WebP er tilladt.');
      return;
    }
      ...
      <div className="rounded-xl border border-border bg-card p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {PHOTO_TYPES.map(type => (
            <label key={type} className="flex flex-col items-center gap-2 p-4 rounded-xl border border-dashed border-border hover:border-primary/50 cursor-pointer transition-colors min-h-[100px] justify-center text-center">
