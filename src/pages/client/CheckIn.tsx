...
export default function ClientCheckIn() {
  ...
  const checkinNum = pendingCheckin?.checkin_number ?? 1;
  ...
  if (!pendingCheckin && clientProfile) {
    return (
      <div className="space-y-5 max-w-lg mx-auto px-4 pb-12">
  ...
  return (
    <div className="space-y-5 max-w-lg mx-auto px-4 pb-24 md:pb-8">
      <div className="flex items-center gap-2">
        <ClipboardCheck className="h-5 w-5 text-primary" />
        <h1 className="text-xl font-bold">Ugentlig Check-in</h1>
      </div>
      <p className="text-sm text-muted-foreground">Check-in #{checkinNum}</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={submitMutation.isPending}
          className="w-full royal-blue-gradient rounded-lg py-2.5 text-sm font-semibold text-primary-foreground hover:opacity-90 flex items-center justify-center gap-2 disabled:opacity-50">
