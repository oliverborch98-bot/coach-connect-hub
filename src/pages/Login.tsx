...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // AuthContext will handle the redirection logic based on the session change
    } catch (err: any) {
          ...
          {!showRequestForm && !showForgotPassword ? (
            ...
            >
              <form onSubmit={handleSubmit} className="space-y-6 liquid-glass p-10 rounded-[2.5rem] border border-white/10 shadow-3xl relative overflow-hidden group">
                ...
                <div className="space-y-3 relative z-10">
                  ...
                  <div className="relative group/input">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-white/5 bg-white/5 px-12 py-4 text-base text-foreground placeholder:text-muted-foreground/20 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:bg-white/[0.08] transition-all"
                ...
                <div className="space-y-3 relative z-10">
                  ...
                  <div className="relative group/input">
                    <input
                      ...
                      className="w-full rounded-2xl border border-white/5 bg-white/5 px-5 py-4 pr-12 text-base text-foreground placeholder:text-muted-foreground/20 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:bg-white/[0.08] transition-all"
                      placeholder="••••••••"
                      required
                    />
                    <button
                ...
                {error && (
                  <motion.p initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="text-sm font-bold text-destructive text-center uppercase tracking-wider">{error}</motion.p>
                )}
              ...
              {resetSent ? (
                <div className="liquid-glass rounded-[2.5rem] border border-white/10 p-10 text-center space-y-8">
                  ...
                  <button
                    ...
                    className="min-h-[44px] flex items-center justify-center text-[14px] font-black uppercase tracking-[0.2em] text-primary hover:text-primary/80 transition-all border-b border-primary/20 pb-1"
                  >
                    TILBAGE TIL LOGIN
                  </button>
                </div>
                ...
                <form onSubmit={handleForgotPassword} className="space-y-6 liquid-glass rounded-[2.5rem] border border-white/10 p-10">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-black uppercase tracking-[0.2em] text-muted-foreground/60 px-1">Din Email</label>
                    <input
                      type="email"
                      value={forgotEmail}
                      onChange={e => setForgotEmail(e.target.value)}
                      className="w-full rounded-2xl border border-white/5 bg-white/5 px-5 py-4 text-base text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                  ...
                  <button
                    ...
                    className="w-full royal-blue-gradient rounded-2xl py-5 text-sm font-black uppercase tracking-[0.2em] text-black hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 mt-2"
                  >
              ...
              {requestSent ? (
                <div className="liquid-glass rounded-[2.5rem] border border-white/10 p-10 text-center space-y-8">
                  ...
                  <button
                    ...
                    className="min-h-[44px] flex items-center justify-center text-sm font-black uppercase tracking-[0.2em] text-primary hover:text-primary/80 transition-all border-b border-primary/20 pb-1"
                  >
                    TILBAGE TIL LOGIN
                  </button>
                </div>
                ...
                <form onSubmit={handleRequestAccess} className="space-y-5 liquid-glass rounded-[2.5rem] border border-white/10 p-10 max-h-[80vh] overflow-y-auto scrollbar-hide">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-3">
                      <label className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground/60 px-1">Fulde Navn</label>
                      <input
                        type="text"
                        value={reqName}
                        onChange={e => setReqName(e.target.value)}
                        className="w-full rounded-2xl border border-white/5 bg-white/5 px-5 py-4 text-base"
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-black uppercase tracking-[0.2em] text-muted-foreground/60 px-1">Email</label>
                      <input
                        type="email"
                        value={reqEmail}
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-black uppercase tracking-[0.2em] text-muted-foreground/60 px-1">Mobilnr.</label>
                      <input
                        type="tel"
                        value={reqPhone}
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-black uppercase tracking-[0.2em] text-muted-foreground/60 px-1">Din Motivation</label>
                      <textarea
                        value={reqMessage}
                        onChange={e => setReqMessage(e.target.value)}
                        className="w-full rounded-2xl border border-white/5 bg-white/5 px-5 py-4 text-base min-h-[100px] resize-none"
                  ...
                  <button
                    ...
                    className="w-full royal-blue-gradient rounded-2xl py-6 text-sm font-black uppercase tracking-[0.2em] text-black hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 mt-4 outline-none group min-h-[64px]"
                  >
                    {isSubmittingRequest ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-5 w-5 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />}
